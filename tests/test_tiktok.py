...
def test_tiktok_download_unauthorized(client):
    ...
    response = client.post('/tiktok-download', data={'url': 'https://www.tiktok.com/@user/video/1234567890'})
...
def test_tiktok_download_no_url(auth_client):
    ...
    response = auth_client.post('/tiktok-download', data={})
...
def test_tiktok_download_invalid_url(auth_client):
    ...
    response = auth_client.post('/tiktok-download', data={'url': 'https://invalid.com'})
    assert response.status_code == 400
    assert b'Invalid TikTok URL' in response.data

def test_tiktok_download_cached(auth_client):
    """Test that cached TikTok content can be retrieved"""
    # Setup cache
    cache = MediaCache('tiktok')
    test_url = 'https://www.tiktok.com/@user/video/test123'
    test_media = {
        'id': 'test123',
        'title': 'Test TikTok',
    ...
    # Test download
    response = auth_client.post('/tiktok-download', data={'url': test_url})
    assert response.status_code == 200
    assert b'test_user' in response.data
    assert b'TikTok Download Results' in response.data
    assert b'video' in response.data.lower() or b'img' in response.data.lower()

    # Cleanup
    test_file.unlink()

def test_tiktok_download_new(auth_client):
    """Test that new TikTok content can be downloaded"""
    test_url = 'https://www.tiktok.com/@user/video/test456'
    response = auth_client.post('/tiktok-download', data={'url': test_url})

    # Since we can't actually download from TikTok in tests,
    # we expect a 500 error with a specific message
    assert response.status_code == 500
    assert b'Failed to download TikTok content' in response.data